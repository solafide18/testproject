    ...
    public class VesselController : BaseController
    {
        ...
        public async Task<IActionResult> ExcelExport()
        {
            ...
            using (var fs = new FileStream(Path.Combine(FilePath, sFileName), FileMode.Create, FileAccess.Write))
            {
                ...
                row.CreateCell(10).SetCellValue("Is Geared");
                row.CreateCell(11).SetCellValue("Is Active");

                excelSheet.DefaultColumnWidth = PublicFunctions.ExcelDefaultColumnWidth;

                mcsContext dbFind = new mcsContext(DbOptionBuilder.Options);

                var tabledata = dbContext.vessel.Where(o => o.organization_id == CurrentUserContext.OrganizationId);
                ...
                foreach (var Value in tabledata)
                {
                    ...
                    row.CreateCell(7).SetCellValue(Value.vehicle_model);
                    ...
                    row.CreateCell(10).SetCellValue(Convert.ToBoolean(Value.is_geared));
                    row.CreateCell(11).SetCellValue(Convert.ToBoolean(Value.is_active));
