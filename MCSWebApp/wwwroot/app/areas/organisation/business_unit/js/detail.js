$(document).ready(function () {
    ...
    var basePath = "/Organisation/BusinessUnit";
    var apiPath = "/api" + basePath;

    $("#ParentBusinessUnit").select2({
        ajax:
        {
            url: apiPath + "/select2",
            headers: {
        ...
        $('#parent_business_unit_id').val(data.id);
        $('#parent_business_unit_name').val(data.text);
    });
            ...
            $.ajax({
                ...
                if (result !== null) {
                    if (result.success && result.data !== null) {
                        ...
                        $('#business_unit_name').val(result.data.business_unit_name);
                        $('#parent_business_unit_id').val(result.data.parent_business_unit_id);
                        $('#parent_business_unit_name').val(result.data.parent_business_unit_name);

                        $('#ParentBusinessUnit').append(new Option(
                            $('#parent_business_unit_name').val(),
                            $('#parent_business_unit_id').val(), true, true)
                        ).trigger('change');

                        if (result.data.id !== null) {
                            $('#ParentBusinessUnit').prop('disabled', true);
                        }
                    }
